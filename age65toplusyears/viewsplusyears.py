...
from .models import Disease, Symptom , DiseaseSymptom,Medicine,MedicineDisease
...
def process_symptoms(request):
    if request.method == 'POST':
       ######for string
        ...
        return render(request, 'w_diseaseplusyears.html', {'matched_diseases': matched_diseases})
    else:
...
def treatment(request, disease_id):
    ...
    return render(request, 'w_treatmentplusyears.html', {'disease': disease, 'medicines': medicines})

def get_symptomsplusyears(request, disease_id):
...
def get_suggestionsplusyears(request):
